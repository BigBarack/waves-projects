...
class FDTD():
    ...
    def PML(self, sigmamax=0.17,thickness_denom = 10, pml_order = 1,prof_type = 'polynomial'):

        # thickness needs to be small relatively to nx
        self.thickness = max(1, self.nx // thickness_denom)
        ...
        def profile(index, thickness, sigmamax,pml_order,prof_type):
            return sigmamax * ((index / thickness) ** pml_order) if prof_type == 'polynomial' else (
                    sigmamax * (1 - np.exp(-pml_order * (index / thickness)**4)))

        # only uses the values in the pml layer so that the non pml layer doesnt get affected
        for i in range(self.thickness):
            sigma_value = profile(i, self.thickness, sigmamax,pml_order,prof_type)
...
problem.PML(sigmamax=0.15,thickness_denom=6,pml_order=12,prof_type='apolynomial')
problem.iterate()
